...
- set in a fantasty world where the various fantasy races (elves, dwarves, humans, etc) work and live with fantasy creatures
...
- using F Macros where possible because the project originally exceeded the Arduino's RAM limit
  - learned that you can use F() to store strings in flash memory instead of RAM
  - using it with lcd.print(), serial.print(), and serial.println()
  - slightly slower but conserves on RAM space
...
void displayHeadpatsGriffin()
{
  lcd.clear();
  // Line 1: Very happy eagle head and wings
  lcd.print(F(" {^▽^}>> "));
  // Line 2: Excited lion body
  lcd.setCursor(0, 1);
  lcd.print(F(" /\\_/\\~~~>"));
  delay(300);
  // Line 1: Very happy eagle head and wings
...
void displayHeadpatsDragon()
{
  lcd.clear();
  // Line 1: Excited dragon head and wings
  lcd.print(F(" /^◡^\\ ")); // Big smiling eyes
  // Line 2: Wagging tail (happier body)
  lcd.setCursor(0, 1);
  lcd.print(F(" \\_⌒_/~~~>")); // Curved mouth & wavy tail
}
...
void displayHeadpatsPhoenix()
{
  lcd.clear();
  // Line 1: Beaming phoenix head with flames
  lcd.print(F(" (^ᗨ^)>>")); // Wide, joyful eyes
  // Line 2: Extra fiery and excited trail
  lcd.setCursor(0, 1);
  lcd.print(F(" //_\\≧≧≧>")); // More dynamic fire
}
...
// clear LCD, check which creature is currently enabled, and then display that creature (in its playing form)
{
  ...
  if (strcmp(currentSpecies, "Griffin") == 0)
  {
    displayHeadpatsGriffin();
  }
  else if (strcmp(currentSpecies, "Dragon") == 0)
  {
    displayHeadpatsDragon();
  }
  else if (strcmp(currentSpecies, "Phoenix") == 0)
  {
    displayHeadpatsPhoenix();
  }
}
...
void nicknamePrompt()
{
...

  {
    ...
    {
      // clear any remaining chacters from serial once the nickname response has been registered
      Serial.read();
      // reads and discards any leftover additional bytes (ex: newlines, extra keypresses)
      delay(10);
    }

    if (nicknamePromptResponse == 'Y' || nicknamePromptResponse == 'y')
    {
      Serial.print(F("\nWhat would you like to nickname "));
    ...
    {
      Serial.println(F("\nYou're not choosing a nickname."));
...
void generateTraits()
{
  if (digitalRead(button1) == HIGH && (!isGenerated))
  {
    ...
    Serial.println(F("."));
    delay(100);
    Serial.print(F("Their parents are currently "));
    Serial.print(job1);
    Serial.print(F(" and "));
    Serial.print(job2);
    Serial.println(F("."));
...
void foodAndWater()
{
  ...
  // else print a message saying they're already full/no longer thirtsy
  if (currentIndexHunger < 2)
  {
    currentHunger = hunger[currentIndexHunger + 1];
    displayEatingCreature();
    // only display if they're having a snack
    Serial.print(currentNickname);
    Serial.print(F(" is now "));
    Serial.print(currentHunger);
    Serial.println(F("."));
    incrementTrust();
    // increment trust is they're having a snack
  }
  else
  {
    Serial.print(currentNickname);
    Serial.print(F(" is already "));
    Serial.print(currentHunger);
    Serial.println(F(" and will not be having another snack."));
  }

  if (currentIndexThirst < 2)
  {
    currentThirst = thirst[currentIndexThirst + 1];
    // only display if they're having a drink
    Serial.print(currentNickname);
    Serial.print(F(" is now "));
    Serial.print(currentThirst);
    Serial.println(F("."));
    incrementTrust();
    // increment trust is they're having a drink
  }
  else
  {
    Serial.print(currentNickname);
    Serial.print(F(" is already "));
    Serial.print(currentThirst);
    Serial.println(F(" and won't be having another drink."));
  }
  delay(2000);
}
...
// same functionality as foodAndWater() but for tiredness level
{
  ...
  if (currentIndexNap < 2)
  {
    ...
    // only display sleeping creature if creature is taking a nap
    Serial.print(currentNickname);
    Serial.print(F(" is now "));
    Serial.print(currentTiredness);
    Serial.println(F("."));
  ...
  else
  {
    ...
    Serial.print(F(" is already "));
    Serial.print(currentTiredness);
    Serial.println(F(" and won't be thank another nap."));
  }
  incrementTrust();
  delay(2000);
}
...
void headpats()
{
  ...
  else
  {
    ...
    displayPlayingCreature();
    // only display if we're giving headpats
    Serial.print(F("\n"));
...
void parentsPickup()
{
  ...
  Serial.println(F("\nPress Button 1 to care for a new creature."));
}
